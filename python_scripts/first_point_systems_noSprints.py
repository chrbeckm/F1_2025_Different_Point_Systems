...
for j, system in enumerate(point_systems):
    os.makedirs(f"_includes/noSprints/{DNFdiff}/{system['dir']}", exist_ok=True)
    ...
    filename = f"_includes/noSprints/{DNFdiff}/{system['dir']}/constructors_{system['name'].replace(' ', '_')}"
    fig.savefig(filename + ".pdf")
    fig.savefig(filename + ".png", dpi=500)
    plt.close(fig)
